...
function getActivePromos() {
  ...
  // Query untuk mendapatkan promo aktif
  $query = "SELECT * FROM promo WHERE start_date <= '$currentDate' AND end_date >= '$currentDate'";
  $result = mysqli_query($conn, $query);

  // Inisialisasi array promo
  $promos = [];
  while ($row = mysqli_fetch_assoc($result)) {
      $promos[] = $row;
  }
...
function addPromo($data) {
  ...
  // Query untuk menambahkan promo
  $query = "INSERT INTO promo (title, description, category, start_date, end_date, image) 
            VALUES ('$title', '$description', '$category', '$startDate', '$endDate', '$image')";
